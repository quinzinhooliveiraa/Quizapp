...
function Home({ variant = 'v1' }: { variant?: 'v1' | 'v2' }) {
  ...
  const checkout = async (packageId: 'couple' | 'family' = selectedPackage) => {
    ...
    <Link href="/login" className="home-login-link" data-testid="link-home-login">Já tem baralho? Entrar</Link>
      <main className="lp-main">{variant === 'v2' ? <LandingV2Quiz onBuy={() => { void checkout('couple'); }} /> : <>
       <section className="lp-hero"><div className="lp-hero-inner"><div className="lp-hero-copy">
         ...
         <div className="lp-hero-actions"><button onClick={() => void checkout('couple')} className="lp-cta-primary" data-testid="button-hero-cta">Quero começar agora <ArrowRight size={18} /></button><button type="button" onClick={() => void checkout('couple')} className="lp-cta-secondary lp-cta-secondary-button">Quero comprar direto</button></div>
...
function AppExperienceReference() {
  ...
  return <div className="app-viewport">
    ...
    {welcomeOpen && <div className="app-modal-backdrop"><div className="app-modal welcome-app-modal"><button className="app-modal-close" onClick={() => setWelcomeOpen(false)} aria-label="Fechar apresentação" data-testid="button-close-welcome"><X size={18} /></button><div className="welcome-app-mark"><Feather size={19} /></div><p className="modal-eyebrow">antes da primeira carta</p><h2>Como podemos<br /><em>te chamar?</em></h2><p>É só para deixar este espaço um pouco mais seu. Você pode entrar sem preencher nada.</p><input value={buyerName} onChange={e => setBuyerName(e.target.value)} onKeyDown={e => e.key === 'Enter' && startSession()} placeholder="Seu nome" className="app-text-input" data-testid="input-buyer-name" /><button onClick={startSession} className="app-primary-button" data-testid="button-enter-experience">{createSession.isPending ? 'Abrindo seu espaço…' : 'Entrar na experiência'} <ArrowRight size={16} /></button></div></div>}
          ...
          {suggestionOpen && <div className="app-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="suggestion-title"><div className="app-modal"><button className="app-modal-close" onClick={() => { setSuggestionOpen(false); setSuggestionStatus('idle'); }} aria-label="Fechar sugestões" data-testid="button-close-suggestion"><X size={18} /></button>{suggestionStatus === 'sent' ? <><p className="modal-eyebrow">obrigado</p><h2 id="suggestion-title">Sua sugestão<br /><em>chegou até nós.</em></h2><p>Lemos todas, prometido.</p><button onClick={() => { setSuggestionOpen(false); setSuggestionStatus('idle'); }} className="app-primary-button" data-testid="button-close-suggestion-sent">Fechar</button></> : <><p className="modal-eyebrow">ideias e sugestões</p><h2 id="suggestion-title">O que podemos<br /><em>melhorar?</em></h2><p>Conta pra gente o que faltou, o que travou, ou o que você adoraria ver aqui.</p><textarea value={suggestionMessage} onChange={e => setSuggestionMessage(e.target.value)} placeholder="Sua sugestão" className="app-textarea" rows={4} data-testid="input-suggestion-message" /><input type="email" value={suggestionEmail} onChange={e => setSuggestionEmail(e.target.value)} placeholder="Seu email (opcional, pra gente poder responder)" className="app-text-input" data-testid="input-suggestion-email" />{suggestionStatus === 'error' && <p className="checkout-error">Não deu pra enviar agora. Tenta de novo em instantes.</p>}<button onClick={() => void sendSuggestion()} disabled={suggestionStatus === 'sending' || !suggestionMessage.trim()} className="app-primary-button" data-testid="button-send-suggestion">{suggestionStatus === 'sending' ? 'Enviando…' : 'Enviar sugestão'}</button></>}</div></div>}
          {reviewOpen && <div className="app-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="review-title"><div className="app-modal"><button className="app-modal-close" onClick={() => { setReviewOpen(false); setReviewStatus('idle'); }} aria-label="Fechar avaliação" data-testid="button-close-review"><X size={18} /></button>{reviewStatus === 'sent' ? <><p className="modal-eyebrow">muito obrigado</p><h2 id="review-title">Sua avaliação<br /><em>significa muito.</em></h2><p>Pode ser que a gente entre em contato pra pedir permissão de usar seu depoimento na página de vendas.</p><button onClick={() => { setReviewOpen(false); setReviewStatus('idle'); }} className="app-primary-button" data-testid="button-close-review-sent">Fechar</button></> : <><p className="modal-eyebrow">sua opinião</p><h2 id="review-title">Como está sendo<br /><em>sua experiência?</em></h2><div className="review-stars" role="radiogroup" aria-label="Nota de 1 a 5">{[1, 2, 3, 4, 5].map(value => <button key={value} type="button" role="radio" aria-checked={reviewRating === value} onClick={() => setReviewRating(value)} className={`review-star-button ${value <= reviewRating ? 'is-filled' : ''}`} data-testid={`button-review-star-${value}`}><Star size={26} fill={value <= reviewRating ? 'currentColor' : 'none'} /></button>)}</div><textarea value={reviewMessage} onChange={e => setReviewMessage(e.target.value)} placeholder="Conta como foi usar o Perguntas de Conexão" className="app-textarea" rows={4} data-testid="input-review-message" /><input value={reviewName} onChange={e => setReviewName(e.target.value)} placeholder="Seu nome (opcional)" className="app-text-input" data-testid="input-review-name" /><input type="email" value={reviewEmail} onChange={e => setReviewEmail(e.target.value)} placeholder="Seu email (opcional)" className="app-text-input" data-testid="input-review-email" />{reviewStatus === 'error' && <p className="checkout-error">Não deu pra enviar agora. Tenta de novo em instantes.</p>}<button onClick={() => void sendReview()} disabled={reviewStatus === 'sending' || !reviewRating || !reviewMessage.trim()} className="app-primary-button" data-testid="button-send-review">{reviewStatus === 'sending' ? 'Enviando…' : 'Enviar avaliação'}</button></>}</div></div>}
